...
def get_all_line_counts(data: str = STATS) -> list:
    ...
    with open(STATS) as fin: #STATS is the file 
        return list(map(lambda x: int(x.split()[0]), fin.read().splitlines()))

 # remember why list(map(lambda( is useful https://book.pythontips.com/en/latest/map_filter.html       
...
def create_stats_report(data=None):
    ...
    return STATS_OUTPUT.format(**stats) # format dictionaries in Python using str.format(**mapping).