...
def get_longest_state(data, reverse=False):
    """Receives data, which can be the us_state_abbrev dict or the states
    list (see above). It returns the longest state measured by the length
    of the string"""
    if type(data) == dict:
        states = data.keys() #dict_keys
    elif type(data) == list:
        states = data
    else:
        raise ValueError
    
    return max(sorted(states,reverse=reverse), key=len) #works for both dict_keys and list 

# data=us_state_abbrev
# get_longest_state(data)
# data=states
# get_longest_state(data)
