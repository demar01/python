...
# inspect.getsource(choose)

# def iprb():
#     k, m, n = map(int, open("Chapter_Rosalind/iprb/rosalind_iprb.txt").read().split())
#     return 1.0 - (choose(n,2) + n*m/2 + choose(m,2) / 4) / choose(k+m+n,2)

def iprb():
    k, m, n = map(int, open("Chapter_Rosalind/iprb/rosalind_iprb.txt").read().split())
    # homozygous dominant (k), heterozygous (m), or homozygous recessive (n)
    pop = k+m+n
